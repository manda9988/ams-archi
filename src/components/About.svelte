<!-- about.svelte -->

<div id="about" class="about">
	<div class="para-about">
		<div class="paraIntro">
			<p>
				Sint ipsam exercitationem minus illum mollitia nihil quaerat nam laboriosam atque ad alias
				voluptates, possimus, in magni neque. Nobis maxime adipisci vitae. Sint ipsam
				exercitationem.
			</p>
		</div>

		<div class="paraUnder">
			<div class="paraLeft">
				<p>
					Cumque nemo corrupti consectetur! Excepturi mollitia, necessitatibus, iusto amet
					architecto molestiae pariatur, corrupti eius autem iure et odit repellendus. Earum
					explicabo quasi doloremque sint beatae, voluptas recusandae at. Eveniet, suscipit
					repudiandae alias modi quia quisquam maxime nemo consequuntur culpa nobis aliquam, iste
					doloremque porro ad ea nihil commodi cumque deleniti!
				</p>
			</div>

			<div class="paraRight">
				<p>
					Inventore doloribus ipsum nulla eveniet ipsam vitae harum optio sapiente. Totam nostrum
					nisi, autem sunt dolorum perspiciatis? Ipsa aliquid a non, incidunt recusandae optio
					temporibus porro voluptatum aut maiores ab error eveniet? Obcaecati distinctio explicabo
					quis vitae recusandae quidem facilis dolores, quae iusto inventore exercitationem eligendi
					debitis modi fuga aspernatur, qui molestiae vero optio tempora alias! Possimus inventore
					labore, harum excepturi reprehenderit beatae.
				</p>
				<p>
					Perferendis vitae voluptas totam eveniet amet minima perspiciatis saepe, debitis repellat
					ratione tempore tempora quia suscipit, quam aspernatur fugit a temporibus quibusdam
					eligendi! Itaque delectus veniam impedit iure unde rerum quidem alias. Nesciunt veritatis
					atque quam, quia soluta qui libero obcaecati asperiores repellat officiis magnam ipsam
					eveniet odio sequi recusandae dicta magni?
				</p>
			</div>
		</div>
	</div>
</div>

<style>
	@import '../styles/about.scss';
</style>
